...
void LCD_Init(void)
{
	...
	//LCD_command(0x40);	//Sets Display to start at first character of first Line
	//_delay_ms(3);
}
...
void LCD_send_enable(void)
{
	...
	_delay_us(2);
	LCD_command_port &= ~(1<<EN);
	_delay_us(100); //according to Arduino library, commands need at least 37 us to go through
}
...
void LCD_command (char cmnd)
{
	...
	LCD_data_port = (cmnd) >> 4; //outputs high nibble of hex command code to PORTD and the LCD’s data bus
	LCD_send_enable();
	LCD_data_port = cmnd & 0x0F; //outputs low nibble of hex command code to PORTD and the LCD’s data bus
	LCD_send_enable();
...
void LCD_char(char char_data)
{
	...
	LCD_send_enable();
	LCD_data_port = char_data & 0x0F;	//outputs char to PORTD and the data bus of the LCD
	LCD_send_enable();
	_delay_ms(2);				//give LCD time to complete write
}
